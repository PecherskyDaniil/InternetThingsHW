...
4. [H-мост](#Hbridge)
5. [Калибровка движения](#MotionCalibration)
6. [Невесомость акселерометра](#Axelerometr)
7. [I2C](#I2C)
...
# MotionCalibration
### Суть работы
>Вам нужно написать прошивку, которую можно использовать для калибровки движения без перепрошивки. Для этого мы будем использовать Bluetooth модуль, который мы будем использовать как UART, работающий по воздуху, и приложение, которое предоставляет соединение  с bluetooth модулем. В приложении нам понадобиться вариант взаимодействия  gamepad. Используйте кнопки X, B, A, Y, <, >, ^, v для того, чтобы пользователь смог откалибровать движение


### Схема подключения
В стандартной машинке bluetooth модуль подключен RX к 13 пину и TX к 8 пину.

### Скетч
Скетч для прошивки микроконтроллера Arduino и код питона лежат в папке Motion_calibration репозитория. В приложении для старта калибровки нужно нажать на крестик. Для завершения нужно нажать на треугольник. Для смены полярности нажать на кадрат.

### Демонстрация работы
Демонстрации работы скетча и схемы доступно по ссылке:
[https://disk.yandex.ru/i/QTirPFCXuQE-GQ](https://disk.yandex.ru/i/QTirPFCXuQE-GQ)
...
### Скетч
Скетч для прошивки микроконтроллера Arduino Nano лежит в папке Axelerometr.

### Демонстрация работы
Демонстрации работы скетча и схемы доступно по ссылке:
[https://disk.yandex.ru/i/ZtgMkj4WfYNo0A](https://disk.yandex.ru/i/ZtgMkj4WfYNo0A)

# I2C
### Суть работы
>Используйте протокол I2C для создания чата между slave устройствами (3 slave устройства). Функции:
>* Написать сообщение одному адресату. Человек вводит адресата (имя) сообщение в Serial monitor на slave.  Имя адресата переводиться в адрес устройства. Таблицу соответствия имен и адресов можно хранить в виде массива. Сообщение считывается и хранится в переменной на Slave. Микроконтроллер master периодически опрашивает slave'ов, если у slave есть что-нибудь есть для отправки (человек ввел сообщение в serial), он отправляет на master количество данных (длину сообщения). Получив длину сообщения master запрашивает у slave это количество данных. Получив данные от отправителя master  переправляет полученное адресату. У адресата должно быть выведен отправитель и сообщение.﻿
>* Ответить последнему написавшему не указывая его адрес.
>
>Все устройства должны быть подключены к одной шине. В качестве отклика предоставить, код, демонстрацию и описание протокола связи, для того чтобы другие разработчики могли реализовать свое устройство и подключить к вашей сети.
### Описание притокола связи
3 устройства подключены к одной шине и имеют адреса a,b,c. При обращении к устройству нужно указать в Serial сначала адрес (символ a,b или c), а затем сообщение. При получении сообщения оно выведится в Serial, после чего адрес отправителя сохранится и в течении 60 секунд вы можете написать ему ответ. Если 60 секунд прошли вы вновь можете написать сообщение новому адресу.
### Демонстрация работы
Интерактивная демонстрации работы скетча и схемы доступны по ссылке:
[https://www.tinkercad.com/things/d6rlBB5oqdQ/editel?sharecode=hO8b2n13ARwOdfZ67XMnOoX53w2BaXDE1h4ibLRKCcA](https://www.tinkercad.com/things/d6rlBB5oqdQ/editel?sharecode=hO8b2n13ARwOdfZ67XMnOoX53w2BaXDE1h4ibLRKCcA)
